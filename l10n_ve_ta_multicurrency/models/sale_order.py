...
class SaleOrder(models.Model):
    """
    EN: Extend sale.order with multicurrency fields for manual rate and fiscal totals.
    ES: Extiende sale.order con campos multidivisa para tasa manual y totales fiscales.
    """
    _inherit = 'sale.order'
    ...
    def _compute_l10n_ve_ta_multicurrency_show_summary(self):
        """
        EN: Always show the fiscal summary panel for sale orders.
        ES: Siempre muestra el panel fiscal para presupuestos de venta.
        """
        for order in self:
    ...
    def _get_l10n_ve_ta_multicurrency_rate(self):
        ...
        if not target_currency or company_currency == target_currency:
            return 1.0
    ...
    def _prepare_invoice(self):
        """
        EN: Transfer custom exchange rate settings to the invoice when created from a sale order.
        ES: Transfiere la configuración de tasa de cambio a la factura al ser creada desde un presupuesto.
        """
...
class SaleOrderLine(models.Model):
    """
    EN: Extend sale.order.line with multicurrency fiscal amounts.
    ES: Extiende sale.order.line con montos fiscales multidivisa.
    """
    ...
    def _onchange_product_id_l10n_ve_ta_multicurrency_hook(self):
        """
        EN: Onchange on product_id to apply our custom multicurrency rate.
            Calls super() safely in case the parent defines the method.
        ES: Onchange en product_id para aplicar la tasa multidivisa.
    ...
    def _compute_l10n_ve_ta_multicurrency_line_amounts(self):
        ...
        for line in self:
            ...
            is_exempt = not bool(line.tax_id)
            ...
            nominal = (line.product_uom_qty or 0.0) * price_unit
